...
```{r}
simulate_pois_dyn_rate <- function(lambda) {
  ...
  return(
    lapply(
      1:k,
      function(i) rpois(1, grid$lambda[i]*cell_size^2)
    ) %>% unlist()
  )
}
...
simulate_ppp_on_grid <- function(grid, poisson_displacement=0) {
  ...
  #   Matrix of points
  k <- nrow(grid)
  n <-  simulate_pois_dyn_rate(grid$lambda) # BETTER WAY OF DOING THIS?
...
# Prior:
prior.sim <- replicate(n, simulate_pois_dyn_rate(prior.grid$lambda)) %>% melt() %>%
  group_by(Var2) %>% summarize(mean=mean(value), std=sd(value))
prior.sim

# Posterior:
posterior.sim <- replicate(n, simulate_pois_dyn_rate(posterior.grid$lambda)) %>% melt() %>%
  group_by(Var2) %>% summarize(mean=mean(value), std=sd(value))
posterior.sim
```

```{r}
ggplot(prior.sim) + geom_histogram(aes(x=mean))
```
