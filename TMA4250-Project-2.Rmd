...
- Pines: The estimated L-function is mostly a straight line coresponding to the theoretical function, implying that a Poisson point process is a suitable model.


## c)
For a homogeneous Poisson point process on a window $W\subseteq \mathbb{R}^m$, we have the convenient result:
\begin{equation}
  \left[\mathbf{x}_1. \cdots, \mathbf{x}_n | N(W)=n\right] \sim U(W)
\end{equation}
